...
class RelevesController extends Controller
{
    ...
    public function relevesnote($matricule, $annee_acad, $semestre)
    {
        ...
        //somme total des coeficient capialisé
        $coefuevalideMajeur = (is_numeric($coefuevalideMajeur)) ? $coefuevalideMajeur : 0;
        $coefuevalideMineur = (is_numeric($coefuevalideMineur)) ? $coefuevalideMineur : 0;

        // Calcul des moyennes pondérées pour les UE majeures et mineures
        $totalCoefMajeur = array_sum(array_column($ueMajeur, 'coef'));
        $totalCoefMineur = array_sum(array_column($ueMineur, 'coef'));
        // Vérification et conversion des moyennes et coefficients en nombres
        foreach ($ueMajeur as &$ue) {
            $ue->Moyenne = floatval(str_replace(',', '.', $ue->Moyenne)); // Convertir les moyennes en nombres
            $ue->coef = intval($ue->coef); // Convertir les coefficients en entiers
        }

        foreach ($ueMineur as &$ue) {
            $ue->Moyenne = floatval(str_replace(',', '.', $ue->Moyenne)); // Convertir les moyennes en nombres
            $ue->coef = intval($ue->coef); // Convertir les coefficients en entiers
        }

        // Calcul des moyennes pondérées pour Majeur
        $totalCoefMajeur = array_sum(array_column($ueMajeur, 'coef'));
        if ($totalCoefMajeur != 0) {
            ...
            $moyennePondereeMajeur = 0;
        }

        // Calcul des moyennes pondérées pour Mineur
        $totalCoefMineur = array_sum(array_column($ueMineur, 'coef'));
